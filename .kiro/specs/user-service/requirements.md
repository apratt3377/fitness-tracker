...
- **AdminController**: The REST controller exposing the admin `/api/v1/admin/users` endpoints
...
- **UserPrincipalResponse**: The internal DTO returned to the Auth Service after successful authentication, containing `id`, `username`, and `roles`
...
- **Soft_Delete**: Setting `deleted_at = 'T'` on a `UserEntity` row; Hibernate automatically excludes such rows from all queries
...
7. WHEN a `UserCreateRequest` contains a `username` exceeding 50 characters or a `password` exceeding 255 characters, THE User_Service SHALL return `400 Bad Request` and SHALL NOT persist any record.
...
**User Story:** As an admin user, I want to list all registered user accounts, so that I can review who has access to the system.

#### Acceptance Criteria

1. WHEN an admin user sends `GET /api/v1/admin/users`, THE AdminController SHALL return `200 OK` with a JSON array of `UserResponse` objects representing all active (non-soft-deleted) users; if no active users exist, the response SHALL be an empty array, not a `404`.
2. THE `UserResponse` objects in the list SHALL contain only `id` and `username`; credential fields (password hash and any secret material) SHALL be omitted from every entry in the list.
3. THE AdminController SHALL exclude soft-deleted users from the response regardless of when the soft-delete was performed, so that deactivated accounts are invisible to admin listings.

---

### Requirement 5: Admin Soft-Delete User Account

**User Story:** As an admin user, I want to soft-delete a user account by UUID, so that the account is deactivated without losing historical data.

#### Acceptance Criteria

1. WHEN an admin user sends `DELETE /api/v1/admin/users/{id}` with a valid UUID for an existing active user, THE AdminController SHALL return `204 No Content`.
...
4. IF the `{id}` path variable does not match any active user — whether because the UUID is absent from the database or because the account was already soft-deleted — THE AdminController SHALL return `404 Not Found`.
5. IF the `{id}` path variable is not a valid UUID format, THE AdminController SHALL return `400 Bad Request`.
...
1. THE `UserEntity` SHALL store a `roles` field typed as `UserEntity.Role` with exactly two valid values: `USER` and `ADMIN`; the field SHALL be non-null on every persisted record.
2. WHEN a `UserCreateRequest` is processed and the `roles` field is absent, `null`, blank, or not one of the recognized values `"USER"` or `"ADMIN"`, THE UserService SHALL persist the `UserEntity` with `roles` set to `Role.USER`.
3. WHEN `POST /internal/v1/identity/authenticate` returns `200 OK`, THE `UserPrincipalResponse` SHALL include a `roles` field equal to the stored `UserEntity.roles` enum value for that account, so the Auth Service can embed the correct role claim in the JWT.
...
1. THE `UserEntity` SHALL use Hibernate's `@SoftDelete` annotation configured with `columnName = "deleted_at"` and `converter = TrueFalseConverter.class`; a value of `'T'` in `deleted_at` SHALL mark the record as deleted, and `'F'` SHALL mark it as active.
2. WHILE a user record has `deleted_at = 'T'`, THE User_Service SHALL exclude that record from all repository query results regardless of which field is queried; no application-level read operation SHALL return a soft-deleted entity.
...
3. THE AdminController SHALL expose all admin endpoints under the path prefix `/api/v1/admin/users/`.
...
2. THE OpenAPI specification served by the User_Service SHALL document all endpoints under `/api/v1/users/` and `/api/v1/admin/users/` including their HTTP methods, path parameters, request body schemas, and response schemas.
...
5. WHEN a `DELETE /api/v1/admin/users/{id}` request references a UUID with no matching active user, THE AdminController SHALL return `404 Not Found`.
